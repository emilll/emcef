...
public interface ContribuableRepository extends JpaRepository<Contribuable, Integer> {
    ...
    @Query(value = "SELECT count(*)  FROM  factureselonspecification c WHERE c.ifu = :ifu", nativeQuery = true)
    ...
    @Query(value = "SELECT c.id, date_activation, date_enregistement, nim, status  FROM  contribuable a, installations b, machineinstallees c WHERE  c.id_installation_id = b.id AND b.contribuable_id = a.id AND a.ifu = :ifu", nativeQuery = true)
